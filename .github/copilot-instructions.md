...
Quick architecture
------------------
- **Backend**: Laravel v12 (app/*, routes/*) with Sanctum for API token auth (not Fortify).
- **API**: RESTful endpoints in `routes/api.php` with Sanctum + custom middleware (auth.active, token.not.revoked).
- **Frontend**: Inertia + React (resources/js/*). Pages in `resources/js/pages/**/*.tsx`, resolved by `resources/js/app.tsx`.
- **Bundling**: Vite + Laravel Sail (Docker-based dev environment with PostgreSQL, Redis, Mailpit).
- **Generated guidance**: `laravel/boost` may read/write guideline files via `boost:update`/`boost:install`.

Key commands
------------
### Development with Sail (Recommended)
- Start Sail: `docker compose up -d`
- Run artisan: `docker compose exec -T laravel.test php artisan <command>`
- Run tests: `docker compose exec -T laravel.test php artisan test`
- Logs: `docker compose logs -f laravel.test`
- API runs on `http://localhost`, Vite on `http://localhost:5173`

### Local Development (without Sail)
- Setup: `composer install && npm install && php artisan migrate && npm run build`
- Dev (concurrent): `composer dev` (serves Laravel + runs `npm run dev`)
- Tests: `php artisan test` (uses PostgreSQL, same as Sail)

### Frontend
- Dev: `npm run dev` (Vite HMR on :5173)
- Build: `npm run build`
- SSR: `npm run build:ssr` + `php artisan inertia:start-ssr`

Important project conventions
-----------------------------
### API Authentication (Sanctum)
- Login: `POST /api/login` (email + password) → returns `{ token: "...", token_type: "Bearer" }`
- Logout: `POST /api/logout` (requires Bearer token) → revokes token
- Protected routes use `auth:sanctum` + custom middleware:
  - `token.not.revoked`: Checks token not revoked
  - `auth.active`: Checks user is_active=true
...
- **Sanctum**: This project uses Laravel Sanctum for API tokens (not sessions/cookies).
  - Login endpoint: `POST /api/login` validates email + password, returns Bearer token.
  - Protected routes: Use `auth:sanctum` middleware + custom `token.not.revoked` (check revoked tokens) + `auth.active` (check is_active=true).
  - Token revocation: `POST /api/logout` revokes the current token via `$user->currentAccessToken()->delete()`.
  - Example controller: [app/Http/Controllers/Api/AuthController.php](app/Http/Controllers/Api/AuthController.php) shows login, logout, me patterns.
- **Middleware Order**: Always place `auth:sanctum` first, then `token.not.revoked`, then `auth.active` for security.
- **User Status**: Check `is_active` field in User model; inactive users fail auth with 403 Forbidden.
- Use Laravel's built-in authorization features (gates, policies, Sanctum, etc.).
...
## Pest 4 + Sanctum API Testing

- Pest v4 is a huge upgrade to Pest and offers: browser testing, smoke testing, visual regression testing, test sharding, and faster type coverage.
- **API Testing**: Use `postJson()`, `getJson()` for testing Sanctum-protected endpoints.
- **Authentication in tests**: Create user + token: `$user = User::factory()->create(); $token = $user->createToken('test')->plainTextToken;`
- **Protected requests**: Pass Bearer token: `$this->getJson('/api/endpoint', ['Authorization' => 'Bearer ' . $token])`
- Browser tests should live in `tests/Browser/`.
- Use the `search-docs` tool for detailed guidance on utilizing these features.

### API Test Patterns
- Test valid credentials, invalid credentials, inactive users, rate limits, token lifecycle
- Example: [tests/Feature/Auth/LoginTest.php](tests/Feature/Auth/LoginTest.php) - login with valid creds, inactive user, rate limit
- All auth tests pass (48 tests): LoginTest (7), LogoutTest (7), MeTest (9), + existing Fortify tests
